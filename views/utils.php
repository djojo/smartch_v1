...
function getResponsablePedagogique($groupid, $courseid, $sessionid = null)
{
    global $DB;
...
ORDER BY u.lastname ASC';
    ...
    if (!empty($userids)) {
        ...
        // Completions e-learning : whitelist + exclusion par nom + deletioninprogress=0
        ...
        // Completions face2face par user dans la session
        if ($session) {
    ...
    // Total e-learning : whitelist + exclusion par nom + deletioninprogress=0
    $totalElearningWithCompletion = (int) $DB->count_records_sql('
        ...
        AND cm.id NOT IN (' . $excludedByNameSql . ')
    ', null);
    // Total face2face : MIN(nb_plannings, nb_face2face) par section
...
ORDER BY u.lastname ASC';
    ...
    // Bordure en bas des lignes 1, 2 et 3 sur toute la largeur (colonnes fixes incluses)
...
ORDER BY 
    ...
    if(!$actualsessions){
        ...
        $actualAttempts = [];   
        //on va chercher les tentative sur les sessions en cours
        foreach($actualsessions as $actualsession){
            foreach($attempts as $attempt){
                // si la tentative est entre la date de début et la date de fin de la session
                if($attempt->timemodified > $actualsession->startdate && $attempt->timemodified < $actualsession->enddate){
                    array_push($actualAttempts, $attempt);
                }
            }
        }
...
function getCompletionPourcent($courseid, $userid = null, $groupid = null)
{
    ...
    // séances présentielles : résoudre le groupe si non fourni
    if (!$groupid) {
    ...
    // face2face : MIN(nb_plannings, nb_face2face) par section + completions réelles par user
    $totalFace2face = 0;
    $completedFace2face = 0;
    if ($groupid) {
        $session = $DB->get_record('smartch_session', ['groupid' => $groupid]);
        if ($session) {
            $sectionStats = $DB->get_records_sql(
            ...
            foreach ($sectionFinished as $s) {
                $completedFace2face += min($s->nb_plannings, $s->nb_face2face);
            }
        }
    }

    $total = $totalElearning + $totalFace2face;
    if ($total == 0) return 0;
    $completed = $completedElearning + $completedFace2face;
...
function face2face_unmark_completed($cm, $userid) {
    global $DB, $CFG;
    ...
    if ($completion->is_enabled($cm) && $face2face->completionpassed) {
        ...
        $record->coursemoduleid = $cm->id;
        $record->userid = $userid;
        $record->completionstate = COMPLETION_INCOMPLETE;
        $record->timemodified = $now;
        $record->viewed = 1;
